...
function observationSummary(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const { date } = observed_on_details;
...
function observationsByPrivacy(data, privacy) {
  ...
  if (typeof privacy === 'string') {
    for (let i in data.results) {
      if (data.results[i].privacy && data.results[i].privacy.toLowerCase() === privacy) {
        obj.privacy = privacy;
        arr.push(obj);
      }
    }
    ...
    for (let i in data.results) {
      if (data.results[i].privacy === null) {
        obj.privacy = null;
        arr.push(obj);
      }
    }
...
function transformObservation(original) {
  let obj = {
    ...
    isResearchQuality: original.quality_grade === 'research', // Set to true if quality_grade is 'research', false otherwise
    geoCoords: original.location.split(',').map(parseFloat).reverse(), // Convert location to an array of numbers [longitude, latitude]
    photoUrls: original.photos.map((photo) => photo.url), // Extract the URL from each photo object
    photosCount: original.photos.length, // Get the count of photo URLs
    user: `@${original.user.login_exact}` // Add '@' prefix to the user's login_exact name
  };
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (eq !== undefined) {
    return data.results.filter(({ positional_accuracy }) => positional_accuracy === eq);
  } else if (gte !== undefined && lte !== undefined) {
    return data.results.filter(
      ({ positional_accuracy }) => positional_accuracy >= gte && positional_accuracy <= lte
    );
  } else if (gte !== undefined) {
    return data.results.filter(({ positional_accuracy }) => positional_accuracy >= gte);
  } else if (lte !== undefined) {
    return data.results.filter(({ positional_accuracy }) => positional_accuracy <= lte);
  }
...
function getUserStats(data) {
  ...
  data.results.forEach(function (element) {
    ...
    obj.totals.observations += user.observations_count;
    obj.totals.journals += user.journal_posts_count;
    obj.totals.species += user.species_count;
  });

  obj.averages.observations = obj.totals.observations / obj.count;
  obj.averages.journals = obj.totals.journals / obj.count;
