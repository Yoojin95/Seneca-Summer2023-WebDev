...
function normalizeVariable(value) {
  ...
  value = value.trim().toUpperCase(); // remove leading whitespace
  value = value.replace(/[ \t.]+/g, '_'); // matches one or more occurrences of spaces, tabs, or dots

  return value;
}

console.log(normalizeVariable('abc')); // 'ABC'
console.log(normalizeVariable(' ABC ')); // 'ABC'
console.log(normalizeVariable('ABC')); // 'ABC'
console.log(normalizeVariable('A BC')); // 'A_BC'
console.log(normalizeVariable(' A bC  ')); // 'A_BC'
console.log(normalizeVariable('A   BC')); // 'A_BC'
console.log(normalizeVariable('A.BC')); // 'A_BC'
console.log(normalizeVariable(' A..  B   C ')); // 'A_B_C'
...
function createIframe(src, height, allowFullScreen) {
  var iframe = `<iframe src="${src.trim()}"`;
  if ((typeof height === Number || String) && height > 0 && height % 1 === 0) {
...
function formatCoords(...values) {
  ...
  for (let i = 0; i < values.length; i++) {
    try {
      if (i < values.length - 1 && normalizeCoord(values[i]) !== 'Invalid') {
        newCoords += normalizeCoord(values[i]) + ', ';
      } else if (normalizeCoord(values[i]) !== 'Invalid') {
        newCoords += normalizeCoord(values[i]);
      }
    } catch (error) {
      newCoords += '';
...
function generateLicenseLink(licenseCode, targetBlank) {
  let htmlLink = `<a href="https://creativecommons.org/licenses/`;

  try {
    licenseCode = licenseCode.toLowerCase();
    var newCode = licenseCode.match(/b.*$/g);

    switch (newCode[0]) {
      case 'by':
        htmlLink += `${newCode[0]}/4.0/"`;
        if (targetBlank) {
          htmlLink += ` target="_blank"`;
        }
        htmlLink += `>Creative Commons Attribution License</a>`;
        break;
      ...
      case 'by-nc':
        ...
        htmlLink += `>Creative Commons Attribution-NonCommercial License</a>`;
        break;
      ...
      case 'by-sa':
        ...
        htmlLink += `>Creative Commons Attribution-ShareAlike License</a>`;
        break;
      ...
      case 'by-nd':
        ...
        htmlLink += `>Creative Commons Attribution-NoDerivs License</a>`;
        break;
      ...
      case 'by-nc-sa':
        ...
        htmlLink += `>Creative Commons Attribution-NonCommercial-ShareAlike License</a>`;
        break;
      ...
      case 'by-nc-nd':
        ...
        htmlLink += `>Creative Commons Attribution-NonCommercial-NoDerivs License</a>`;
        break;
    }
  } catch (error) {
    htmlLink = '<a href="https://choosealicense.com/no-permission/">All Rights Reserved</a>';
  }
...
function pureBool(value) {
  ...
  try {
    if (value === true) {
      ...
      retVal = false;
    } else if (value.match(/^[t|o|y|v]/gi)) {
      retVal = true;
    } else if (value.match(/^[n|f]/gi)) {
      retVal = false;
    }
  } catch (error) {
    throw 'invalid value';
...
function every() {
  ...
  let returnVal;
  try {
    for (let i = 0; i < arguments.length && breakout === 0; i++) {
  ...
  try {
    for (let i = 0; i < arguments.length && breakout === 0; i++) {
      if (pureBool(arguments[i]) === true) {
        trueCnt++;
      } else if (pureBool(arguments[i]) === false) {
        falseCnt++;
      }
    }
  } catch (error) {
